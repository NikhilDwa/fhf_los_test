...
from common.file_headers import core_column_name
from common.core_acceptance_constants import core_acceptance_input


class UseCaseToDataGenerateServiceCore:
    ...
    def get_acceptance_data(self, use_case):
        ...
        for column in core_column_name:
            if column in core_acceptance_input:
                if (
                    isinstance(core_acceptance_input[column], list)
                    and len(core_acceptance_input[column]) == 2
                    and all(isinstance(i, int) for i in core_acceptance_input[column])
                ):
                    acceptance_data_row[column] = random.randint(
                        core_acceptance_input[column][0], core_acceptance_input[column][1]
                    )
                elif (
                    isinstance(core_acceptance_input[column], str)
                    and "," in core_acceptance_input[column]
                ):
                    options = core_acceptance_input[column].split(",")
                    acceptance_data_row[column] = random.choice(options).strip()
                else:
                    acceptance_data_row[column] = core_acceptance_input[column]

        if acceptance_data_row["VEHICLE_CONDITION"] == "New":
            acceptance_data_row["VEHICLE_YEAR"] = random.choice(["2024", "2025"])
        elif acceptance_data_row["VEHICLE_CONDITION"] == "Used":
            acceptance_data_row["VEHICLE_YEAR"] = str(random.randint(2015, 2023))
    ...
    def write_data_to_csv(self):
        ...
        with open(self.file_path, mode="a", newline="") as file:
            fieldnames = [field for field in core_column_name]
    ...
    def generate_use_case_data(self):
        ...
        self.write_header_to_csv(core_column_name)
