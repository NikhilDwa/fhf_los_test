...
from aspire_acceptance_test import constants as c

input_file_name = "aspire_acceptance_input.csv"
output_file_name = "aspire_acceptance_output.csv"
...
with open(output_file_name, mode="w", newline="") as csvfile:
    ...
    writer.writerow(c.aspire_acceptance_column_headers)
...
for row in csv_data:
    ...
    for column in c.aspire_acceptance_column_headers:
        if column in c.aspire_acceptance_input:
            if (
                isinstance(c.aspire_acceptance_input[column], list)
                and len(c.aspire_acceptance_input[column]) == 2
            ):
                acceptance_data_row[column] = random.randint(
                    c.aspire_acceptance_input[column][0], c.aspire_acceptance_input[column][1]
                )
            elif (
                isinstance(c.aspire_acceptance_input[column], str)
                and "," in c.aspire_acceptance_input[column]
            ):
                options = c.aspire_acceptance_input[column].split(",")
                acceptance_data_row[column] = random.choice(options).strip()
            else:
                acceptance_data_row[column] = c.aspire_acceptance_input[column]
    ...
    with open(output_file_name, mode="a", newline="") as file:
        fieldnames = [field for field in c.aspire_acceptance_column_headers]
