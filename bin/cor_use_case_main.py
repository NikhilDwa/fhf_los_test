...
from services.usecase_to_data_generatecore_service import UseCaseToDataGenerateServiceCore
...
def main():
    ...
    if user_input == "y":
        try:
            file_path = StringUtils().get_file_with_string("cor")
            if file_path:
                ...
                output_file_path = PathUtils.get_output_folder().joinpath(
                    Path(f"cor_acceptance_{today_date}.csv")
                )
                UseCaseToDataGenerateServiceCore(
